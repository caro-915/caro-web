...
<div class="max-w-4xl mx-auto px-4 py-6 md:py-8">
    ...
    <h1 class="text-lg font-bold mt-2">
        Conversation – 
        <a href="{{ route('annonces.show', ['annonce' => $conversation->annonce->id, 'slug' => $conversation->annonce->slug ?? Str::slug($conversation->annonce->titre)]) }}" 
        ...
        @foreach($messages as $message)
            ...
            <div class="flex {{ $mine ? 'justify-end' : 'justify-start' }}">
                <div class="max-w-[70%] rounded-2xl px-3 py-2 text-sm
                    {{ $mine ? 'bg-gray-800 text-white' : 'bg-gray-100 text-gray-800' }}">
                    {{ $message->body }}
    ...
    <div class="bg-white rounded-2xl shadow p-4">
        <form method="POST" action="{{ route('messages.store', $conversation) }}" id="messageForm">
            ...
            <button type="submit"
                    class="mt-3 inline-flex items-center justify-center px-4 py-2 rounded-full bg-gray-800 text-white text-sm font-semibold hover:bg-gray-900">
                Envoyer
            </button>
        </form>
    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function buildMessageElement(msg) {
        ...
        bubble.className = 'max-w-[70%] rounded-2xl px-3 py-2 text-sm ' +
            (isMine ? 'bg-gray-800 text-white' : 'bg-gray-100 text-gray-800');
